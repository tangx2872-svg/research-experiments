...
| 时间 | 项目 | 实验内容 | 进度 |
| --- | --- | --- | --- |
| 2026-09 | [Django LMS 教学管理系统](2026/09/django-lms/README.md) | 开源部署、中文界面、课程管理、作业提交与评分 | 核心流程完善；30 项测试通过；本机 MySQL 新增迁移已执行成功 |
...
- **首页功能入口**：作业中心按课程与状态集中查看任务，成绩反馈中心查看分数、评语与历史提交；教师和学生分别展示自己的数据。

### 平台首页

深青色与浅灰配色、自制书本标识（非官方校徽）、课程卡片和个人工作台。样式与图标本地加载，截图来自实际 8000 首页。

![平台首页](2026/09/django-lms/docs/images/01-home.png)
...
系统检查、30 项自动化测试和独立 SQLite 环境的完整浏览器流程已通过。根据本机终端反馈，本轮 4 个 MySQL 新增迁移均已执行成功；MySQL 环境的完整页面流程仍待复验。
